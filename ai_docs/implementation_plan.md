...
- **Dockerization**: Use the existing `Dockerfile` in the repo, modified to start the API automatically.
...
### Phase 5: Post-Deployment Optimization (Planned)
1.  **Race Control Integration**: Connect the backend message feed to the front-end sidebar.
2.  **Responsiveness**: Audit the Timing Tower for tablets and smaller screens.
3.  **Aesthetics**: Add micro-interactions and transitions to make the leaderboard feel "alive."

### Phase 6: Advanced Telemetry UI (Requested)
Implementing the deep telemetry features requested for a true "Broadcast" experience:
- **Header**: Live Lap Counter (e.g., LAP 28 / 63).
- **Driver Rows**:
    - **Tyres**: Visual compound icons (Soft/Medium/Hard) + stint age.
    - **Pit Status**: Bold red "PIT" indicator for cars in the lane.
    - **Sector Timings**: Actual numbers (26.431) instead of just dots, with color highlighting.
    - **Lap Times**: Dedicated column for the Last Lap time.
    - **Fastest Lap**: ⏱️ emoji and purple text for the session's overall fastest lap holder.
